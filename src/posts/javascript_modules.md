...
6. This technique is incompatible with TypeScript. The TypeScript compiler cannot know what functions are available and what are their types. You could
...
Modules provide a solution to the import problem. They solve the challenges listed above and make large scale JavaScript/TypeScript projects practical.
...
When NodeJS was created in 2009, Node users needed a way to load scripts from external files.
...
To solve the issue, a new way to load code was needed. The *CommonJS* module system (or *CJS* for short) was thus invented.
...
The RequireJS implementation is essentially the specification of AMD as it is the most used loader. It has a lot of extra features on top of the basic `require`:
...
> ⚠️ Depending on the module system you are using, `require` can take a string or an array as its first argument!
...
It is mainly a way to export files, not to import them and is commonly used by libraries that try to support both Node and the browser.
...
While UMD works both in Node and Browser, the syntax for its header is very verbose. Usually, this header is generated by a build system or bundler.
...
ESM appeared in 2015 when other module types were already established.
In ESM, `export` and `import` are reserved keywords which can break existing code.
A system was needed to allow compatibility between ESM and older module types.
...
Scripts imported from a module using `import` are assumed to be modules.

In NodeJS, to activate the "module mode", you can add `"type": "module"` in your package.json:
...
It takes many JS files as input with the CommonJS import style and bundles them into one large file.
...
The main drawback of Vite is that it needs ESM modules to work. If you are using old libraries which is CommonJS or AMD, you need a conversion step before using Vite.
...
Libraries needs to support both types of users. Let's take [JQuery](https://github.com/jquery/jquery) as an example.
...
For people that don't use a bundler, JQuery bundles its own code into one file which defines global variables (Internally, The JQuery project seems to use rollup with [swc](https://swc.rs/)). The bundler produces a file with [the *UMD* format](https://code.jquery.com/jquery-3.7.1.js) so that people can use JQuery in the browser and in NodeJS.
